...
from utils.auth import admin_required, editor_required
...
def create_lookup(lookup):
    ...
    elif lookup in ('compositores', 'arreglistas', 'ubicaciones'):
        new_id = execute(
            f'INSERT INTO {table} (nombre, notas) VALUES (%s, %s)',
            (data.get('nombre'), data.get('notas'))
        ) if lookup != 'ubicaciones' else execute(
            f'INSERT INTO {table} (nombre, descripcion) VALUES (%s, %s)',
...
def delete_lookup(lookup, item_id):
    if lookup not in TABLES:
        return jsonify({'error': 'Catálogo no encontrado'}), 404
    table, pk = TABLES[lookup]
    # Protect reserved records
    if lookup == 'instrumentos':
        row = query(f'SELECT nombre FROM {table} WHERE {pk} = %s', (item_id,))
        if row and row[0]['nombre'] == 'Gidoia':
            return jsonify({'error': 'Gidoia ezin da ezabatu — sistema-erregistroa da'}), 403
